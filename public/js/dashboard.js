...
// Split from views/dashboard.html to improve performance and code organization
        ...
        async function loadAdvancedStats() {
            ...
            try {
                ...
                const allOrdersRaw = await ordersRes.json();
                ...
                if (tableBody) {
                    const distsRes = await fetch('/api/distributors');
                    const distributors = await distsRes.json();
        ...
        window.rejectPendingReview = async function(reviewId) {
            ...
            try {
                await fetch('/api/testimonials/reject', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ id: reviewId })
                });
        ...
        function updateDistributorStatus(id, status) {
            showConfirm('هل أنت متأكد من تغيير حالة هذا الموزع؟', async () => {
                try {
                    await fetch('/api/distributor/status', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/json'},
        ...
        async function loadDistributorOrders() {
            try {
                ...
                const dRes = await fetch('/api/distributors');
                const distributors = await dRes.json();
        ...
        async function saveCoupon(e) {
            ...
            const url = isEdit ? `/api/coupons/${encodeURIComponent(oldCode)}` : '/api/coupons';
            ...
            try {
                const res = await fetch(url, {
                    method: method,
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ code, type, value, minOrder, maxUses, targetPhone, productIds })
                });
                const data = await res.json();
                if(res.ok) {
                    ...
                    alert(data.error || 'فشل حفظ الكوبون');
                }
            } catch(err) {
        ...
        async function sendMarketingNotification() {
            ...
            try {
                const urlPath = isEdit ? `/api/marketing/notifications/${id}` : '/api/marketing/send-notification';
                const method = isEdit ? 'PUT' : 'POST';

                const res = await fetch(urlPath, {
                    method: method,
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ title, message, url, imageUrl, target, targetPhone })
                });
                
                if(res.ok) {
                    showToast(isEdit ? '✅ تم تحديث الحملة بنجاح' : '🚀 تم إطلاق الحملة بنجاح');
                    showNotifListView(); // العودة للسجل
                } else {
                    const data = await res.json();
                    alert(data.error || 'فشل معالجة الإشعار');
                }
            } catch(e) {
        ...
        async function deleteNotification(id) {
            ...
            try {
                const res = await fetch(`/api/marketing/notifications/${id}`, { method: 'DELETE' });
                if(res.ok) {
        ...
        async function saveAsCustomTemplate() {
            ...
            try {
                const res = await fetch('/api/marketing/custom-templates', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
        ...
        async function deleteCustomTemplate(id) {
            ...
            try {
                await fetch(`/api/marketing/custom-templates/${id}`, { method: 'DELETE' });
                loadCustomTemplates();
            } catch(e) { console.error(e); }
        }
        ...
        async function uploadNotifImage(input) {
            ...
            try {
                ...
                const data = await res.json();
                if(data.success) {
                    document.getElementById('notif_image').value = data.imageUrl;
            ...
            try {
                ...
                if(res.ok && data.success) {
                    const successMsg = isEdit 
                        ? '✅ تم تحديث الحملة وإعادة إطلاقها' 
                        : '🚀 تم إطلاق الحملة وإرسال الإشعارات';
            ...
            if (confirm(`هل أنت متأكد من حذف كود الخصم "${code}"؟`)) {
                ...
                .then(data => {
                    if(data.success) {
                        ...
                        alert(data.error || 'فشل حذف الكوبون');
                    }
                })
        ...
        function openCategoryEdit(id) {
            ...
            const viewBtn = document.getElementById('btnViewCategory');
            if (viewBtn) {
                viewBtn.href = '/?app=product.cat.' + cat.id;
        ...
        async function triggerVideoFileUpload(btn) {
            ...
            fileInput.onchange = async (e) => {
                ...
                try {
                    ...
                    const result = await res.json();
                    if (result.url) {
                        document.getElementById('videoUrlInput').value = result.url;
                        previewVideoInput(result.url);
                ...
                reader.onload = async (event) => {
                    try {
                        const res = await fetch('/api/upload', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ name: file.name, data: event.target.result })
                        });
                        const json = await res.json();
                        if (json.url) {
                            updateValImagePreview(container, json.url);
        ...
        async function confirmDeleteProduct(id) {
            showConfirm('هل أنت متأكد من رغبتك في حذف هذا المنتج نهائياً؟', async () => {
                try {
                    const res = await fetch(`/?account=delete_product&id=${id}`, {
        ...
        async function openEditById(id) {
            ...
            viewBtn.href = '/?app=product.show.' + product.id;
            viewBtn.style.display = 'flex';
        ...
        async function generateWithAI() {
            ...
            try {
                ...
                document.getElementById('prod_name').value = result.name;
                document.getElementById('prod_desc').value = result.description;
                
                closeAIModal();
                alert('تم إنشاء محتوى المنتج بنجاح باستخدام الـ AI!');
            } catch (e) {
        ...
        document.getElementById('productForm').addEventListener('submit', async function(e) {
            ...
            checkboxes.forEach(cb => {
                data[cb.name + '_val'] = cb.checked ? 'true' : 'false';
            });
...
// --- Additional Event Listeners & Distributor Management ---
        ...
        async function toggleDistributorProducts(id, phone, nameEl) {
            ...
            if(isHidden) {
                ...
                if(container) {
                    ...
                    try {
                        const res = await fetch('/api/distributor/purchased-products?phone=' + encodeURIComponent(phone));
                        const products = await res.json();
        ...
        function openDistributorModal(dist) {
            ...
            if(listContainer) {
                ...
                fetch('/api/distributor/purchased-products?phone=' + encodeURIComponent(dist.phone))
                    .then(res => res.json())
                    .then(products => {
                        if(products.length === 0) {
...
async function changeOrderStatus(orderId, newStatus) {
    try {
        const response = await fetch('/?account=update_order_status', {
        ...
        } else { showToast('❌ ' + (result.message||'خطأ')); }
...
async function deleteStoreOrder(orderId) {
    showConfirm('هل أنت متأكد من حذف هذا الطلب نهائياً؟ لا يمكن التراجع عن هذا الإجراء وسيتم إعادة المنتجات للمخزون إذا لزم الأمر.', async () => {
        try {
            const response = await fetch('/?account=delete_order', {
                method: 'POST',
                headers: {'Content-Type':'application/json'},
            ...
            if (result.success) {
                ...
                setTimeout(() => {
                    window.location.href = '/?account=dashboard&tab=orders';
                }, 1000);
            } else {
                showToast('❌ ' + (result.message||'خطأ أثناء الحذف'));
            }
        } catch(err) {
...
async function addOrderNote(orderId) {
    ...
    try {
        const response = await fetch('/?account=update_order_notes', {
            method: 'POST', headers: {'Content-Type':'application/json'},
            body: JSON.stringify({orderId, notes: noteText})
        });
        const result = await response.json();
        if(result.success) {
            const order = window.storeOrdersData.find(o => o.id === orderId);
...
async function submitInlineEdit(event, orderId) {
    ...
    try {
        const response = await fetch('/?account=update_order', {
            method: 'POST', headers: {'Content-Type':'application/json'},
            body: JSON.stringify(data)
        });
        const result = await response.json();
        if(result.success) {
            ...
            if(idx > -1) window.storeOrdersData[idx] = result.order;
            showToast('✅ تم حفظ التغييرات بنجاح');
            viewOrder(orderId);
        } else { showToast('❌ ' + (result.message||'خطأ في الحفظ')); }
...
async function submitEditOrder(event, orderId) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (orderIndex > -1) {
                        window.storeOrdersData[orderIndex] = result.order;
                    }
                    showToast('✅ تم تحديث بيانات ومنتجات الطلب بنجاح');
                    viewOrder(orderId); // Refresh UI
                } else {
                    showToast('❌ ' + (result.message || 'حدث خطأ أثناء تحديث الطلب'));
                }
            } catch (err) {
                console.error(err);
...
async function monitorNewOrders() {
    try {
        const res = await fetch(`/api/dashboard/orders-count?_=${Date.now()}`);
        const data = await res.json();
...
// ===== Store Pages Management (Policies & Custom Pages) =====
...
async function loadPagesList() {
    ...
    try {
        ...
        if (pages.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:50px; color:var(--text-muted);"><i class="fas fa-file-alt" style="font-size:30px; display:block; margin-bottom:10px; opacity:0.3;"></i> لا توجد صفحات مضافة حالياً.</td></tr>';
            return;
        }

        tbody.innerHTML = pages.map(p => `
            <tr>
                <td style="font-weight:800; color:var(--text-main);">${p.title}</td>
                <td style="direction:ltr;"><code style="background:#f1f5f9; padding:2px 6px; border-radius:4px; font-size:12px;">/page/${p.slug}</code></td>
                <td>
                    <span style="font-size:11px; font-weight:700; background:#f0fdf4; color:#16a34a; padding:3px 8px; border-radius:8px;">
                        ${p.type === 'policy' ? 'سياسة' : p.type === 'about' ? 'من نحن' : p.type === 'shipping' ? 'سياسة شحن' : p.type === 'general' ? 'صفحة عامة' : 'مخصصة'}
                    </span>
                </td>
                <td>
                    <span style="${p.status === 'public' ? 'background:#ecfdf5; color:#059669;' : 'background:#fef2f2; color:#dc2626;'} padding:4px 10px; border-radius:30px; font-size:11px; font-weight:800;">
                ...
                <td>
                    <div style="display:flex; gap:8px;">
                        ...
                        <button class="btn-icon" onclick="deleteStorePage('${p.id}')" style="color:#ef4444;" title="حذف"><i class="fas fa-trash"></i></button>
                        <a href="/page/${p.slug}" target="_blank" class="btn-icon" style="color:var(--primary);" title="عرض"><i class="fas fa-external-link-alt"></i></a>
                    </div>
                </td>
            </tr>
        `).join('');
    } catch (e) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; color:#ef4444; padding:30px;">فشل تحميل الصفحات</td></tr>';
    }
}

function autoGeneratePageSlug(text) {
    const slug = text.toLowerCase()
        .replace(/[^\u0600-\u06FF\w ]+/g, '') // Keep Arabic and alphanumeric
        .replace(/ +/g, '-');
    document.getElementById('page_slug').value = slug;
}

async function openPageEditor(id = null) {
    switchTab('tab-page-editor');
    ...
    const contentInput = document.getElementById('page_content');
    ...
    if (!id) {
        ...
        try {
            ...
            if (page) {
                ...
                typeInput.value = page.type;
                statusInput.value = page.status;
                
                // Load thumbnail if exists
                if (page.thumbnail) {
                    document.getElementById('page_thumbnail').value = page.thumbnail;
                    document.getElementById('page_image_preview').innerHTML = `<img src="${page.thumbnail}" style="width:100%; height:100%; object-fit:cover; border-radius:12px;">`;
                } else {
            ...
            Swal.fire('خطأ', 'فشل جلب بيانات الصفحة', 'error');
        }
    }
}
...
async function saveStorePage() {
    ...
    try {
        ...
        if (result.success) {
            Swal.fire('تم الحفظ', 'تم حفظ بيانات الصفحة بنجاح', 'success');
            switchTab('tab-pages');
            loadPagesList();
        } else {
            Swal.fire('فشل الحفظ', result.error || 'حدث خطأ غير متوقع', 'error');
        }
    } catch (e) {
...
async function deleteStorePage(id) {
    const confirm = await Swal.fire({
        title: 'هل أنت متأكد؟',
        text: 'لن تتمكن من استرجاع هذه الصفحة بعد الحذف!',
        icon: 'warning',
...
async function handlePageImageUpload(event) {
    ...
    reader.onload = async (e) => {
        ...
        try {
            ...
            const result = await res.json();
            if (result.url) {
                document.getElementById('page_thumbnail').value = result.url;
                document.getElementById('page_image_preview').innerHTML = `<img src="${result.url}" style="width:100%; height:100%; object-fit:cover; border-radius:12px;">`;
            }
        } catch (err) {
            console.error('Upload Error:', err);
        }
...
async function openProductSelectorForPage() {
    ...
    // 2. Create Search UI for Swal
    let productsHTML = products.map(p => `
        <div class="product-mini-item" onclick="insertProductShortcode('${p.id}')" style="display:flex; align-items:center; gap:10px; padding:10px; border:1px solid #eee; border-radius:10px; cursor:pointer; margin-bottom:5px; transition:0.2s;">
            ...
            <div style="text-align:right; flex:1;">
                ...
                <div style="font-size:11px; color:var(--primary); font-weight:700;">${p.price} ₪</div>
            </div>
    ...
    Swal.fire({
        title: 'اختر منتجاً لتضمينه',
        html: `
            <input type="text" id="swalProductSearch" class="input-luxury" placeholder="ابحث عن منتج..." style="margin-bottom:15px;" oninput="filterSwalProducts(this.value)">
            <div id="swalProductList" style="max-height:300px; overflow-y:auto; padding:5px;">
                ${productsHTML}
            </div>
        `,
...
function insertProductShortcode(id) {
    ...
    editor.value = text.substring(0, start) + shortcode + text.substring(end);
    Swal.close();
    editor.focus();
}

function filterSwalProducts(query) {
...
window.insertProductShortcode = insertProductShortcode;
window.filterSwalProducts = filterSwalProducts;
    ...
    try {
        ...
        tbody.innerHTML = pages.map(p => `
            <tr>
                ...
                <td style="font-family:monospace; font-size:12px;">/?page=${p.slug}</td>
                ...
                <td>
                    <div style="display:flex; gap:8px;">
                        <button class="btn-icon" onclick="openPageEditor('${p.id}')" title="تعديل"><i class="fas fa-edit"></i></button>
                        <a href="/?page=${p.slug}" target="_blank" class="btn-icon" title="معاينة" style="display:flex; align-items:center; justify-content:center; text-decoration:none;"><i class="fas fa-eye"></i></a>
...
async function saveStorePage() {
    ...
    try {
        ...
        if(result.success) {
            ...
            alert('فشل حفظ الصفحة: ' + result.error);
        }
...
async function handlePageImageUpload(event) {
    ...
    reader.onload = async function(e) {
        ...
        try {
            ...
            const data = await res.json();
            if(data.url) {
                document.getElementById('page_thumbnail').value = data.url;
                previewArea.innerHTML = `<img src="${data.url}" style="width:100%; height:100%; object-fit:cover; border-radius:12px;">`;
            }
...
async function openBannerPresetsModal() {
    ...
    try {
        const res = await fetch('/api/banner-presets');
        allBannerPresets = await res.json();
...
async function saveCurrentAsPreset() {
    ...
    try {
        const res = await fetch('/api/banner-presets', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name, bgColor, textColor, customCss })
        });
        const data = await res.json();
        if (data.success) {
...
async function savePromotion() {
    ...
    try {
        ...
        if (res.ok && data.success) {
            ...
            showToast('❌ ' + (data.error || 'فشل حفظ العرض'), 'error');
        }
    } catch (e) {
...
function showReelEditor(id = null, updateUrl = true) {
    ...
    if (id) {
        ...
        fetch('/api/reels').then(res => res.json()).then(reels => {
...
async function uploadReelVideoPage(input) {
    ...
    try {
        ...
        const result = await response.json();
        if (result.url) {
            document.getElementById('reelPageVideoUrl').value = result.url;
            updateReelPreview();
...
async function saveReel(event) {
    ...
    try {
        const url = id ? `/api/reels/${id}` : '/api/reels';
        const method = id ? 'PUT' : 'POST';
...
async function saveReelsShuffleSetting() {
    ...
    try {
        const res = await fetch('/api/settings', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ reelsShuffle })
        });
        const data = await res.json();
        if (res.ok && data.success) {
    ...
    reader.onload = async (e) => {
        try {
            ...
            if (confirmResult.isConfirmed) {
                ...
                const result = await res.json();
                if (result.success) {
                    await Swal.fire('تم بنجاح!', result.message, 'success');
...
function copyFeedLink(platform, format = 'csv') {
    ...
    const link = `${protocol}//${host}/api/products/feed?platform=${platform}&format=${format}`;

...
function printOrderInvoice() {
    ...
    printWindow.document.close();
        ...
        async function loadMonthlyReport() {
            ...
            const monthName = monthNames[m - 1] || mon;
            ...
            function filterMonth(ord, yy, mm) {
                const d = new Date(ord.date || ord.createdAt);
            ...
            function calcStats(ordList) {
                ...
                ordList.forEach(o => {
                    (o.items || []).forEach(item => {
                        ...
                        if (!items[key]) items[key] = { name: item.name || 'منتج', qty: 0, total: 0, price: parseFloat(item.price) || 0 };
                        const q = parseInt(item.quantity) || 1;
                        items[key].qty += q;
                        items[key].total += items[key].price * q;
                        const cost = costMap[item.id || item.productId] || 0;
                        costTotal += cost * q;
                    });
                });
